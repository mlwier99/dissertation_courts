...
use "FJD/Datasets/Federal Judiciary Directory.dta", clear 
...
*Nominated Days and Months 
foreach i of numlist 1/6 { 
gen nommonth`i' = nominationdate`i'
tostring nomyear`i', replace
replace nommonth`i' = ustrregexrf(nommonth`i', nomyear`i', "") 
replace nommonth`i' = ustrregexrf(nommonth`i', "-[0-9]*[0-9]$", "") 
replace nommonth`i' = ustrregexrf(nommonth`i', "-", "") 

gen nomday`i' = nominationdate`i'
replace nomday`i' = ustrregexrf(nomday`i', nomyear`i', "") 
replace nomday`i' = ustrregexrf(nomday`i', nommonth`i', "") 
replace nomday`i' = ustrregexrf(nomday`i', "-", "") 
replace nomday`i' = ustrregexrf(nomday`i', "-", "") 

destring nomyear`i', replace 
destring nommonth`i', replace 
destring nomday`i', replace 
}

gen seniorjudge = 0 
gen terminatedjudge = 0 
foreach i of numlist 1/6 { 
replace seniorjudge = 1 if seniorstatusyear`i'!=.
replace terminatedjudge = 1 if terminationyear`i'!=. & nomyear`i+1'==.
}
